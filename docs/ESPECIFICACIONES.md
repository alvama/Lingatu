...
**En las fichas**: las que tienen notas muestran 📝 (`notesIndicatorHtml`, compartida por `cardHtml` y `cardHtmlCompact`, así que sale en **los dos modos de vista**). Es la única forma de saber que ahí hay algo escrito sin abrir el modal, y es un botón: `data-action="notes"` en el listener delegado de `#linksContainer` abre el modal de ese enlace con el foco en las notas. **La nota no se pinta en la ficha** (puede tener cientos de líneas: reventaría la cuadrícula). El indicador vive dentro de `.card-icon-actions`, así que el modo selección (4.21) lo oculta con el mismo criterio que ▲▼✏️🗑️.

**Desde la extensión** (ver sección 8): dos entradas de menú contextual —"Guardar en PinBoard" (contexto de página, hace lo mismo que pulsar el icono) y "Añadir selección como nota en PinBoard" (contexto de selección)—, que llaman a `PinBoardBridge.appendNote`. Si la URL ya está guardada, la nota se añade sin modal y **sin cambiar de pestaña**; si no, se abre el modal precargado con la nota puesta (decisión 33).
...
| `formatNoteBlock(text)` / `appendNoteToText(existing, block)` | Notas (4.23): envuelven un texto en su bloque con encabezado de fecha (`## DD/MM/AAAA`) / lo añaden al final de las notas ya existentes, separado por una línea en blanco y sin espacios sobrantes |
...
Las tres siguientes son las decisiones de las **notas por enlace** (4.23):

33. **Añadir una nota es silencioso; crear el enlace pide confirmación.** Si la URL ya existe, `appendNote` añade la nota directamente —guarda, repinta y resalta la ficha—, sin modal y **sin activar la pestaña de PinBoard**: es una operación aditiva sobre un enlace que el usuario ya curó, no destruye nada, y el usuario está leyendo una página y quiere seguir; la confirmación es el badge ✓ del icono de la extensión. Si la URL no existe, se abre el modal precargado con la nota puesta y se salta a esa pestaña, porque hace falta que el usuario actúe. La asimetría sigue el criterio que ya estableció `prefillAndOpen`: *crear* pasa por confirmación —la categoría es una heurística y puede fallar—, *añadir* a algo existente no necesita ceremonia.
...
| `PinBoardBridge.appendNote({url,note,title,description})` → `{ok, created, id?, title?, category?}` | Método de `window.PinBoardBridge`, usa `findDuplicateUrl` + `formatNoteBlock`/`appendNoteToText` o `prefillAndOpen` | Añade una nota al enlace de esa URL (4.23). Si existe, la añade y guarda —`created:false`, y la extensión se queda donde está y confirma con el badge—; si no existe, abre el modal precargado con la nota puesta —`created:true`, y la extensión salta a la pestaña de PinBoard—. El texto llega ya formateado (una selección, como cita); aquí solo se le pone el encabezado de fecha. Devuelve `{ok:false, reason:"empty-note"}` si falta la URL o la nota |
...
4. Seleccionar texto en una página **ya guardada** → menú contextual → "Añadir selección como nota en PinBoard": la nota debe añadirse **sin cambiar de pestaña**, con el badge ✓ verde como única confirmación. Al volver a PinBoard, la nota está como cita con la fecha de hoy, y repetir la operación añade un **segundo** bloque sin borrar el primero.

...
- **Búsqueda** (`#searchInput`): coincidencia de subcadena (case-insensitive) sobre título + descripción + etiquetas + **notas** (4.23) concatenados. Buscar por algo que tú escribiste sobre un enlace es la mitad del valor de las notas.
...
**Fase 1: texto plano.** El Markdown se guarda y se muestra **tal cual** — es una convención de escritura, no un formato de salida, porque las notas son para leerlas uno mismo. No hay renderizador; el mini-renderizador sigue en el backlog (11.1) y **solo** si el uso real lo pide.
...
| `getFilteredLinks()` | Aplica todos los filtros activos sobre `state.links` (enlaces de la vista aplicada — 4.17, categoría, activos, etiquetas incluidas, etiquetas **excluidas**, búsqueda), sin reordenar (ver nota "Orden" en sección 3) |
...
| `openNotesViewer(id)` / `closeNotesViewer()` | Visor de notas en solo lectura (4.23): pinta título, categoría, dominio y el texto con `textContent`, y lo cierra. Registrado en `OVERLAYS` como cualquier otro overlay |
...
Las cinco siguientes son las decisiones de las **notas por enlace** (4.23):
...
| **Media** | **Fase 2 de las notas: mini-renderizador Markdown** | Cinco construcciones (negrita, cursiva, enlaces, listas, cita) en ~60 líneas de regex, aplicadas **después** de `escapeHtml`, nunca antes — ese orden es lo que lo mantiene a salvo de XSS y respeta que `escapeHtml` sea el único saneador de la app. Solo si la fase 1 (texto plano, ya implementada — 4.23) se queda corta en el uso real. Hoy las notas solo se ven dentro del `<textarea>` del modal, así que un renderizador necesita antes decidir **dónde** se pintan. |
...
**Implementado** (funcionalidad completa en 4.23, decisiones 33-35 de la sección 7, superficie del puente en la 8): el campo `notes` en los cinco sitios que reconstruyen un enlace, el `<textarea>` del modal, el indicador 📝 en las fichas de los dos modos de vista, `appendNote` en el puente, los dos menús contextuales de la extensión (permiso `contextMenus`) y `notes` dentro del texto donde busca `getFilteredLinks()`.

**Fuera de alcance, a propósito:**

- **Renderizar el Markdown**: fase 2, en 11.1 con prioridad media y solo si el uso real lo pide. Las notas se guardan y se muestran tal cual.
...
**Y ya hay una de esas dos superficies escrita**: el visor de notas de PinBoard (`#notesViewerOverlay`, decisión 37) muestra exactamente lo mismo que tendrá que mostrar el panel en la página — título, categoría y el texto tal cual, con los saltos de línea por CSS y el texto insertado con `textContent`. No se puede compartir el código (son dos entornos: la página y la extensión), pero sí el **criterio de presentación**, y conviene que se parezcan también visualmente. El aviso efímero que ya inyecta la extensión al añadir una nota (decisión 36) resuelve además la parte técnica: shadow DOM cerrado, `activeTab` sin permisos nuevos y `textContent`; el panel es esa misma técnica con contenido persistente en vez de efímero.

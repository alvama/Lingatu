...
|---|---|---|
| **Muy alta** | **Ayuda inmediata: estados vacíos que enseñan + `title` en todo lo interactivo** | El problema no es de documentación sino de **descubribilidad** (ver 11.5). Dos cambios de media hora cada uno: (a) cuando no hay *ningún* enlace, `#emptyState` deja de decir "sin resultados" y enseña las tres primeras acciones — crear un enlace, importar los marcadores del navegador, pulsar `/` para buscar; (b) atributo `title` en todo control interactivo que hoy no lo tenga (las categorías ya lo llevan). Máximo retorno por línea de código de todo el backlog. **Requisitos listos para acometer en [`docs/tareas/01-ayuda-inmediata.md`](tareas/01-ayuda-inmediata.md).** |
| **Muy alta** | **Selección múltiple de enlaces + acciones en lote** | Modo selección (casillas por ficha) y una barra de acciones sobre la selección: añadir/quitar etiqueta, cambiar categoría, activar/desactivar, borrar. Es el cuello de botella real cuando la colección crece: hoy etiquetar 30 enlaces son 30 viajes al modal. No necesita modelo de datos nuevo — reutiliza `ensureCategory`/`ensureTag`, `save()` y `render()`. Toca `cardHtml`/`cardHtmlCompact`, así que hay que repasar la tabla de la sección 8. **Requisitos listos para acometer en [`docs/tareas/02-seleccion-multiple-lote.md`](tareas/02-seleccion-multiple-lote.md).** |
| **Alta** | **Importar marcadores del navegador arrastrando el archivo** | Soltar sobre la página el HTML de marcadores que exporta cualquier navegador (Ctrl+Shift+O → Exportar) o el propio `Bookmarks` de Chromium, y parsearlo en el sitio: `DOMParser` para el formato Netscape (`<DT><A HREF>` dentro de `<DL>`, carpetas en `<H3>`), `JSON.parse` para el de Chromium. La ruta de carpetas se convierte en `category`, igual que ya hace `tools/convertir_marcadores.py`. Desemboca en el modal de importación existente (Fusionar / Sustituir todo). **Deja el proyecto más simple**: `tools/` (script Python + su configurador HTML + el JSON de configuración) pasa a ser opcional en el camino de entrada de un usuario nuevo. **Requisitos listos para acometer en [`docs/tareas/03-importar-marcadores-navegador.md`](tareas/03-importar-marcadores-navegador.md).** |
| **Alta** | **Paleta de comandos (Ctrl+K)** | Un único desplegable que busca a la vez enlaces y *acciones* ("Exportar", "Gestionar categorías", "Ir a categoría X"), navegable con flechas y Enter. Es la forma correcta de resolver lo que se descartó como "más atajos de teclado" (11.2): **un atajo en lugar de diez**, y descubrible leyendo en vez de memorizando. Reutiliza `getFilteredLinks()` y el patrón de navegación por flechas ya escrito para las sugerencias de etiquetas (4.20). Efecto secundario valioso: al listar cada acción por su nombre, es en sí mismo un sistema de ayuda. |
| **Alta** | **Búsqueda con operadores** | El mismo `#searchInput`, entendiendo `cat:desarrollo`, `#referencia`, `-#trabajo`, `site:github.com` y `"frase exacta"`. Un parser de ~40 líneas dentro de `getFilteredLinks()`; sin texto con operadores sigue funcionando como la subcadena de siempre (4.3), así que no hay UI nueva ni comportamiento que desaprender. Extra de una línea: si se pega una URL en el buscador, saltar al enlace que ya la tenga (reutiliza `findDuplicateUrl`). |
| **Alta** | **Notas Markdown por enlace, capturadas desde la página** | Campo `notes` acumulativo por enlace, alimentado desde la extensión (incluida la captura de la selección de texto como cita). Convierte PinBoard de "donde guardo enlaces" en "donde guardo lo que sé sobre esos enlaces". **Requisitos detallados en 11.3** — leer antes de implementar, incluye un fallo latente verificado en los importadores. |
...
**R2 — Las notas no se pueden perder en el ciclo exportar/importar.** Requisito explícito, y hay un **fallo latente ya verificado en el código actual**: la exportación usa `JSON.stringify(links)` (`pinboard.html:2514`), que serializa el objeto completo y por tanto incluiría `notes` sola; pero **ambos importadores reconstruyen cada enlace campo por campo con una lista blanca** — `performImportReplace` (`pinboard.html:2532-2542`) y `performImportMerge` (`pinboard.html:2564-2572`). Sin tocarlos, un `notes` exportado correctamente se **descartaría en silencio al importar**. Son dos líneas (`notes: l.notes ? String(l.notes) : ""` en cada uno), pero omitirlas produce pérdida de datos silenciosa, que es el peor tipo. Aplica igual a la exportación autocontenida (11.6).
...
Truco para evitar `<all_urls>` incluso en la fase 1: el `background` puede leer la URL de la pestaña activa con el permiso `tabs` y **poner el recuento en el badge del icono** ("2"), sin inyectar nada en ninguna página; el panel se inyecta solo al pulsar el icono, con `activeTab`. Se conserva casi toda la magia —ves que hay algo anotado ahí— sin pedir permiso para modificar todos los sitios web.

...
- **De archivo a local**: se cargan los datos del navegador y, **si se llegó ahí aplazando la apertura** (el botón «Trabajar con los datos de este navegador» de la pantalla), aparece un aviso con su fecha (`#modeNotice`, bajo la barra de herramientas): *«Estás viendo la última copia guardada en este navegador el 14/08/2026 a las 18:52, no tu archivo `lingatu-datos.json`»*. Dura esa sesión y se puede cerrar. Sale de `enlaces_file_meta_v1.savedAt`, que ya se guardaba.

  > **Este aviso nació permanente y se revisó** (decisión 69). Un banner que no se puede quitar y que ofrece «abrir mi archivo» en cada arranque **discute una decisión que el usuario ya tomó**, que es justo lo contrario de la premisa de 4.29. Lo que R8 quería evitar se sigue evitando por otro camino: **el indicador de la barra superior dice siempre dónde se guarda**, y con un archivo recordado su `title` lleva la fecha de la copia que se está viendo. La información no desaparece; deja de interrumpir.
...
69. **Informar una vez no es lo mismo que insistir, y el aviso de modo local cruzaba esa raya.** R8 pedía un aviso permanente con la fecha mientras se trabajara en el navegador teniendo un archivo recordado, y era razonable escrito sobre el papel: evita creer que se está mirando el archivo. En uso real resultó ser otra cosa — un banner que **no se puede cerrar** y que en cada arranque propone cambiar de modo **le lleva la contraria al usuario sobre una decisión que ya tomó**, y la premisa de 4.29 es exactamente la contraria: el modo es suyo. La corrección no consiste en quitar información sino en **cambiarla de sitio**: el estado permanente vive en el indicador de la barra superior (que ya estaba ahí, siempre visible) y la fecha pasa a su `title`; el banner se reserva para el único caso en que de verdad es un aplazamiento y no una decisión —acabas de decir "ahora no" en la pantalla de apertura—, dura esa sesión y se cierra. Regla general que deja: **un aviso que no se puede cerrar tiene que describir algo que el usuario no ha elegido.**

70. **La accesibilidad de esta interfaz está medida, no estimada — y la primera medición pilló un fallo propio.** Al revisar el diseño con criterios de accesibilidad (contraste, tamaño de objetivo, nombre accesible, foco) se midió elemento a elemento en la aplicación real, en los dos temas, y salieron tres cosas:

    - **La atenuación de los iconos de ficha (decisión 65) incumplía el mínimo de contraste de componentes.** Con opacidad 0,3 el contraste real caía a **1,47:1 en claro y 1,73:1 en oscuro**, muy por debajo del 3:1 que exige WCAG 1.4.11. Y el dato que obliga a rediseñar en vez de retocar: **el gris atenuado parte de 4,83:1 sobre la ficha**, así que *ninguna* opacidad por debajo de 0,75 puede cumplir. El margen entre el color de partida y el mínimo legal solo daba para bajar el tono, no para apagarlo — la idea de "iconos casi invisibles hasta pasar el ratón" era incompatible con la norma desde el principio.
    - **Los objetivos de pulsación**: 54 controles por debajo de los 24×24 px de WCAG 2.2 AA, y casi todos fallaban **por el alto** (22×20, 29×20, y los botones de texto a 16 px), que es el que no se ve venir porque el ancho parecía suficiente. Quedan 6, y son los que la propia norma exceptúa: enlaces en línea dentro de una frase.
    - **Dos textos secundarios** rozaban el 4,5:1 (`4,47` el contador de la barra en claro, `4,43` un enlace del lateral en oscuro). Se corrigió el token `--text-muted` en los dos temas, que es un solo sitio.

    La regla que deja: **contraste y tamaño no se aprueban mirando una captura.** Los tres fallos eran invisibles a ojo —de hecho dos los introdujo una mejora de diseño hecha con buena intención— y los tres salieron a la primera al calcular la luminancia relativa y leer los rectángulos.
...
- **Accesibilidad medida (no auditada por completo).** Se ha comprobado con medición directa, en los dos temas: contraste de texto (≥4,5:1), contraste de componentes (≥3:1), tamaño de objetivo (24×24 px, WCAG 2.2 AA) y nombre accesible de los controles. **Lo que no se ha comprobado** con lector de pantalla real: el orden de lectura, los `aria-live` y el comportamiento de los modales. Y queda una decisión abierta de fondo: **los iconos de acción son emoji** (✏️🗑️📝▲▼), no SVG, así que su aspecto lo decide la fuente del sistema y su color no se puede controlar; las categorías sí usan SVG en línea (`CATEGORY_ICONS`), que es el precedente por si algún día se unifican.
...
Decisión adoptada, por orden de retorno: **estados vacíos que enseñan y `title` en todo lo interactivo** (implementado — [`docs/tareas/01-ayuda-inmediata.md`](tareas/01-ayuda-inmediata.md), y revisado después en 4.8); **panel "?" con la chuleta** (implementado, 4.25; la paleta de comandos —4.22— cubre las acciones y deja solo los gestos, filtros y datos por documentar); **tour interactivo guiado, descartado** (11.2).

**Segunda vuelta, esta vez mirando la aplicación en vez del código** ([`docs/tareas/17-experiencia-de-uso.md`](tareas/17-experiencia-de-uso.md)): catorce estados capturados con una colección de 140 enlaces y a tres anchos de ventana, con el usuario objetivo delante. Salieron seis cosas que ninguna lectura del código habría dado, porque no son errores sino **efectos de conjunto**: cuatro recuentos correctos que juntos hacían dudar de todos, un mensaje útil escondido tras una condición que casi nunca se cumplía, una bienvenida que prometía un archivo inexistente, cientos de iconos compitiendo con los títulos, y ochocientos píxeles de lateral por delante de la colección en ventana estrecha. La conclusión metodológica merece quedarse: **la descubribilidad no se audita leyendo, se audita mirando**.

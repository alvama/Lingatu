...
| **Alta** | **Importar marcadores del navegador arrastrando el archivo** | Soltar sobre la página el HTML de marcadores que exporta cualquier navegador (Ctrl+Shift+O → Exportar) o el propio `Bookmarks` de Chromium, y parsearlo en el sitio: `DOMParser` para el formato Netscape (`<DT><A HREF>` dentro de `<DL>`, carpetas en `<H3>`), `JSON.parse` para el de Chromium. La ruta de carpetas se convierte en `category`, igual que ya hace `tools/convertir_marcadores.py`. Desemboca en el modal de importación existente (Fusionar / Sustituir todo). **Deja el proyecto más simple**: `tools/` (script Python + su configurador HTML + el JSON de configuración) pasa a ser opcional en el camino de entrada de un usuario nuevo. |

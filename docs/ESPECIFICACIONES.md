...
| `getFilteredLinks()` | Aplica todos los filtros activos sobre `state.links` (enlaces de la vista aplicada — 4.17, categoría, activos, etiquetas incluidas, etiquetas **excluidas**, búsqueda con operadores), sin reordenar (ver nota "Orden" en sección 3). Parsea la búsqueda una sola vez con `parseSearchQuery()`, fuera del `filter` |
